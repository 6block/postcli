...
import (
	...
	"github.com/spacemeshos/post/expostrs"
...
func main() {
	...
	if genProof {
		...
		// SavePost to post.bin
		dir := "/tmp/"
		if upbin {
			dir = opts.DataDir
		}
		if err := activation.SavePost(dir, (*types.Post)(proof)); err != nil {
...
func cmdVerifyPos(opts config.InitOpts, fraction float64, logger *zap.Logger) {
	...
	params := expostrs.NewScryptParams(opts.Scrypt.N, opts.Scrypt.R, opts.Scrypt.P)
	verifyOpts := []expostrs.VerifyPosOptionsFunc{
		expostrs.WithFraction(fraction),
		expostrs.FromFile(uint32(opts.FromFileIdx)),
		expostrs.VerifyPosWithLogger(logger),
	}
	if opts.ToFileIdx != nil {
		verifyOpts = append(verifyOpts, expostrs.ToFile(uint32(*opts.ToFileIdx)))
	}

	err = expostrs.VerifyPos(opts.DataDir, params, verifyOpts...)
	switch {
	case err == nil:
		log.Println("cli: POS data is valid")
	case errors.Is(err, expostrs.ErrInvalidPos):
		log.Fatalf("cli: %v\n", err)
	default:
